...
| statement-initial `try`, `throw` in a file or block statement list used as names (`try = 1`, `throw = 1`) | identifiers (before Roku OS 9.4; L1 asks such code to be rewritten) | statement keywords, so the line is an error | provisional choice | BS-ERR-005, BS-LEX-025 |
| statement-initial `continue`, `library` in a file or block statement list used as names | identifiers (L1 says nothing about these words as names) | statement keywords, so the line is an error | provisional choice | BS-LEX-025 |
