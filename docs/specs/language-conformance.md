...
| BS-EXP-018 | `NOT` is a prefix operator below comparisons and above `AND` (`not a = b` = `not (a = b)`, `not a and b` = `(not a) and b`). | EVT §Operators, §Logical and bitwise operators | baseline | documented | `unary_expression` (NOT level) | `BS-EXP-018: NOT below comparison and above AND` | n/a | covered | — | EXP-13. The fixture's `not not a` line relies on the nesting choice of BS-EXP-028. |
...
| BS-EXP-028 | Prefix operators nest: a prefix operator may start the operand of another prefix operator, and each applies to the operand that follows it (`- -x` = `-(-x)`, `+-1` = `+(-1)`, `- not x` = `-(not x)`). `++x` and `--x` inside an expression, which the grammar also parses as two prefix operators, stay non-contractual (BS-STMT-004). | L1 silent (no Level 1 example nests prefix operators); Level 4: BrighterScript @ `01a359c6` `src/parser/Parser.ts` `prefixUnary()` recurses for `-` and `+` without an error diagnostic | unknown | provisional | `unary_expression` (operand `unary_expression`) | `BS-EXP-028: nested prefix operators` | n/a | covered | — | Session 05-1 re-audit (R-A-03). Provisional like BS-EXP-027: nesting is a placement choice for documented prefix operators, which needs no extra rule; the nested shape is the choice. The retired KL-002 was measured on this shape. The `not not a` line of the BS-EXP-018 fixture relies on the same choice. BrighterScript lexes `++`/`--` as single tokens, so its evidence does not cover `++x`/`--x`. |
